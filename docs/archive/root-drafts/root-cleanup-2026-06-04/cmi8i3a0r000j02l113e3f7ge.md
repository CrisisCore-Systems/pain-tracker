---
title: "How to Build a Healthcare PWA That Works Offline When Patients Need It Most"
seoTitle: "How to Build a Healthcare PWA That Works Offline When Patients Need It Most"
seoDescription: "A practical guide to building a healthcare PWA that keeps symptom tracking usable offline with local persistence, service workers, and resilient sync."
...
This guide explains how to build a healthcare PWA that still works when hospital Wi-Fi fails, connectivity drops, or a patient needs to log symptoms immediately.

It covers the offline-first decisions behind Pain Tracker: local persistence, service-worker caching, resilient sync, and failure handling that keeps core tracking usable under stress.

If you want to see the patient-facing outputs this architecture supports, start with the [Daily Pain Tracker Printable](https://www.paintracker.ca/resources/daily-pain-tracker-printable), the [Monthly Pain Tracker Printable](https://www.paintracker.ca/resources/monthly-pain-tracker-printable), [What to Include in a Pain Journal](https://www.paintracker.ca/resources/what-to-include-in-pain-journal), or go straight to the [PainTracker download page](https://www.paintracker.ca/download).
...
I'm going to break this down into four main chunks: how we store data (spoiler: we use two databases), the service worker magic that keeps everything running, background sync that catches up when you're connected again, and the PWA manager that makes it all feel smooth. Each section has actual code from production, because generic examples don't help anyone build real things.
