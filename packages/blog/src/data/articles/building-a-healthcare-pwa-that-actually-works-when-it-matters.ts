...
const article: ArticleData = {
  ...
  sections: [
    ...
    },
    {
      ...
      paragraphs: [
        'Claiming a healthcare PWA works offline usually means one of three things: the app opens without internet, data entry works without internet, or all features work without internet. These are meaningfully different. The first is a marketing claim. The second is a basic requirement. The third is what reliable health tracking actually demands.',
        'For a pain tracking app, offline completeness means the core write path, the read path, and the export path all function without connectivity. Trend analysis, medication correlation, and functional impact summaries must run locally. If any of these features require a server, the app fails the patients who most need it: those with unstable access.',
      ],
    },
    {
  ...
  faqs: [
    {
      ...
      answer: 'For pain tracking specifically, no. All entry storage, analytics, and export can run locally. A backend is only warranted if you need cross-device sync, practitioner-facing portals, or institution-level reporting. Each of those requires explicit consent architecture to handle health data responsibly.',
    },
    {
