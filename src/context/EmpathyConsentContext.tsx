...
export const EmpathyConsentProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [privacyLevel, setPrivacyLevelState] = useState<EmpathyConsentState['privacyLevel']>('personal');

  const grantConsent = useCallback(() => setConsentGranted(true), []);
  const revokeConsent = useCallback(() => setConsentGranted(false), []);
  const setPrivacyLevel = useCallback((level: EmpathyConsentState['privacyLevel']) => setPrivacyLevelState(level), []);

  return (
    <EmpathyConsentContext.Provider value={{ consentGranted, privacyLevel, grantConsent, revokeConsent, setPrivacyLevel }}>
      {children}
