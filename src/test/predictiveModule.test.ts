...
interface TestPain { baselineData: { pain: number }; id: string; timestamp: Date; }
interface TestMood { mood: number; notes: string; timestamp: Date; emotionalClarity: number; emotionalRegulation: number; hopefulness: number; context: string; copingStrategies: string[]; anxiety: number; socialSupport: string; }
const mkPain = (p: number): TestPain => ({ baselineData: { pain: p }, id: `p${p}`, timestamp: new Date() });
const mkMood = (m: number): TestMood => ({ mood: m, notes: '', timestamp: new Date(), emotionalClarity:5, emotionalRegulation:5, hopefulness:5, context:'', copingStrategies:[], anxiety:0, socialSupport:'none' });
