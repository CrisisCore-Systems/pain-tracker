...
import { getQueue, queueFailedRequest, processQueue, type OfflineQueueDeps } from '../lib/offline/queue';

// Minimal in-memory cache mock
class MemoryCache {
  store = new Map<string, Response>();
  async match(key: string | Request) { return this.store.get(typeof key === 'string' ? key : key.url); }
  async put(key: string | Request, response: Response) { this.store.set(typeof key === 'string' ? key : key.url, response); }
}
...
describe('offline queue', () => {
  ...
  it('queues failed request and retrieves it', async () => {
    const req = new Request('https://example.com/api/pain-entries', { method: 'POST', body: JSON.stringify({ a: 1 }) });
  ...
  it('processes queue removing successful items and leaving failures', async () => {
    ...
    await queueFailedRequest(new Request('https://x.test/fail', { method: 'POST', body: '2' }), deps);
