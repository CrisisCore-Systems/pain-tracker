...
export class AdvancedAnalyticsEngine {
  ...
  calculateCorrelationMatrix(
    ...
    if (entries.length < 10) {
      console.warn('Insufficient data for reliable correlation analysis (need at least 10 entries)');
      return [];
    }
  ...
  scoreInterventions(entries: PainEntry[]): InterventionScore[] {
    ...
    interventionMap.forEach((data, intervention) => {
      ...
      const avgPainAfter = data.painAfter.length > 0
        ? data.painAfter.reduce((a, b) => a + b, 0) / data.painAfter.length
        : avgPainBefore;
      
      const avgReduction = avgPainBefore - avgPainAfter;
      const successRate = data.painAfter.length > 0
        ? (data.successes / data.painAfter.length) * 100
        : 0;
      
      // Calculate effectiveness score (0-100)
      const effectivenessScore = Math.min(100, Math.max(0,
  ...
  generateWeeklyClinicalBrief(
    ...
    if (trend === 'worsening') {
      concerns.push(`Pain levels increasing - average up ${Math.abs(painChange).toFixed(1)} points from last week`);
    }
    if (avgPain >= 7) {
    ...
    if (effectiveInterventions.length > 0) {
      recommendations.push(`Continue effective interventions: ${effectiveInterventions.join(', ')}`);
    }
    if (trend === 'improving') {
  ...
  private findNextEntry(
    ...
    return remainingEntries.find(entry => {
  ...
  private categorizeIntervention(intervention: string): InterventionScore['type'] {
    ...
    if (lower.includes('medication') || lower.includes('pill') || lower.includes('drug') || 
        lower.includes('ibuprofen') || lower.includes('tylenol') || lower.includes('acetaminophen')) {
      return 'medication';
    }
    
    if (lower.includes('therapy') || lower.includes('massage') || lower.includes('acupuncture') ||
        lower.includes('chiropractic') || lower.includes('physical')) {
      return 'treatment';
    }
    
    if (lower.includes('meditation') || lower.includes('breathing') || lower.includes('mindfulness') ||
        lower.includes('relaxation') || lower.includes('distraction')) {
