...
export function categorizeWisdom(insight: string): 'practical' | 'emotional' | 'spiritual' | 'relational' | 'self-knowledge' {
  const lower = insight.toLowerCase();
  if (lower.includes('relationship') || lower.includes('people') || lower.includes('connect')) return 'relational';
  if (lower.includes('feel') || lower.includes('emotion') || lower.includes('heart')) return 'emotional';
  if (lower.includes('meaning') || lower.includes('purpose') || lower.includes('spiritual')) return 'spiritual';
  if (lower.includes('myself') || lower.includes('i am') || lower.includes('identity')) return 'self-knowledge';
  return 'practical';
}

export function assessApplicability(insight: string): number {
  const actionWords = ['can', 'will', 'should', 'need to', 'must', 'always', 'never'];
  const actionCount = actionWords.reduce((count, w) => count + (insight.toLowerCase().includes(w) ? 1 : 0), 0);
...
export function assessTransformativeLevel(insight: string): number {
  ...
  const intensityCount = intensityWords.reduce((count, w) => count + (insight.toLowerCase().includes(w) ? 1 : 0), 0);
...
export async function extractWisdomInsights(userId: string, _painEntries: unknown[], moodEntries: MoodEntry[]): Promise<WisdomInsight[]> {
  const insights: WisdomInsight[] = [];
  const wisdomEntries = moodEntries.filter(e => {
    const n = e.notes.toLowerCase();
    return n.includes('learned') || n.includes('realized') || n.includes('understand now') || n.includes('wisdom') || n.includes('insight');
  });
...
export function buildWisdomProfile(userId: string, painEntries: unknown[], moodEntries: MoodEntry[]): WisdomProfile {
  ...
  void userId; void painEntries; // reserved for future deeper integration
  return profile;
}
...
export function calcSpiritual(entries: MoodEntry[]): number {
  if (!entries.length) return 50;
  const words = ['meaning', 'purpose', 'transcend', 'transcendent', 'faith', 'spiritual', 'bigger than', 'grateful'];
  let hits = 0;
  for (const e of entries) { const n = e.notes.toLowerCase(); if (words.some(w => n.includes(w))) hits++; }
  const clarity = entries.reduce((s, e) => s + e.emotionalClarity, 0) / entries.length;
...
export function calcSelfKnowledge(entries: MoodEntry[]): number {
  if (!entries.length) return 50;
  const words = ['i feel', 'i notice', 'i realized', 'i understand', 'i learned', 'aware', 'noticing'];
  let hits = 0;
  for (const e of entries) { const n = e.notes.toLowerCase(); if (words.some(w => n.includes(w))) hits++; }
  const clarity = entries.reduce((s, e) => s + e.emotionalClarity, 0) / entries.length;
...
export function calcWisdomApplication(entries: MoodEntry[]): number {
  ...
  let count = 0;
  for (const e of entries) { const n = e.notes.toLowerCase(); if (actions.some(w => n.includes(w))) count++; }
  const regulation = entries.reduce((s, e) => s + e.emotionalRegulation, 0) / entries.length;
...
export function calcWisdomSharing(entries: MoodEntry[]): number {
  ...
  let hits = 0;
  for (const e of entries) { const n = e.notes.toLowerCase(); if (shareWords.some(w => n.includes(w))) hits++; }
...
export function calcIntegratedWisdom(entries: MoodEntry[]): number {
  ...
  let integrated = 0;
  for (const e of entries) { const n = e.notes.toLowerCase(); if (insightWords.some(w => n.includes(w)) && applyWords.some(w => n.includes(w))) integrated++; }
