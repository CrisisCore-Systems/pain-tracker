...
export function buildPredictiveModel(
  ...
  const recentMood =
    moodEntries.slice(-7).reduce((s, e) => s + e.mood, 0) /
    Math.max(1, Math.min(7, moodEntries.length || 1));
  return {
    empathyForecast: [
      {
        timeframe: 'next week',
        predictedEmpathyLevel: Math.max(0, Math.min(100, recentMood * 10)),
        confidenceInterval: { min: 40, max: 80 },
        influencingFactors: [],
    ...
    riskPrediction: [
      {
        riskType: 'compassion_fatigue',
        riskLevel: Math.max(0, Math.min(100, recentPain * 10)),
        timeToRisk: 14,
        earlyWarningSignals: [],
