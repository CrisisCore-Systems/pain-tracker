...
class FakeAnalyticsService {
  async calculateQuantifiedEmpathy(_userId: string, _pain: any[], _mood: any[]) {
    ...
    return {
      emotionalIntelligence: { selfAwareness: 120, selfRegulation: -5, motivation: 50, empathy: 50, socialSkills: 50, emotionalGranularity: 50, metaEmotionalAwareness: 50 },
      compassionateProgress: { selfCompassion: 50, selfCriticism: 10, progressCelebration: 50, setbackResilience: 50, hopefulness: 50, postTraumaticGrowth: 50, meaningMaking: 50, adaptiveReframing: 50, compassionFatigue: 50 },
      empathyKPIs: { validationReceived: 50, validationGiven: 50, emotionalSupport: 50, understandingFelt: 50, connectionQuality: 50, empathicAccuracy: 50, empathicConcern: 50, perspectiveTaking: 50, empathicMotivation: 50, boundaryMaintenance: 50 },
      humanizedMetrics: { courageScore: 50, vulnerabilityAcceptance: 50, authenticityLevel: 50, growthMindset: 50, innerStrength: 50, dignityMaintenance: 50, purposeClarity: 50, spiritualWellbeing: 50, lifeNarrativeCoherence: 50 }
    } as any;
  }
  async generateEmpathyInsights() { return []; }
  async generateEmpathyRecommendations() { return []; }
}
...
describe('EmpathyMetricsCollector', () => {
  it('throws when consent required but not granted', async () => {
    ...
    await expect(c.collect([], [], { userId: 'u1', consentGranted: false })).rejects.toThrow(/Consent required/);
  });
  ...
  it('sanitizes notes and counts redactions', async () => {
    ...
    const res = await c.collect(pain as any, mood as any, { userId: 'u1', consentGranted: true, sanitize: true });
  ...
  it('injects noise when differentialPrivacy true', async () => {
    ...
    const r1 = await c.collect(pain as any, mood as any, { userId: 'u1', consentGranted: true, differentialPrivacy: true, noiseEpsilon: 0.5 });
    const r2 = await c.collect(pain as any, mood as any, { userId: 'u1', consentGranted: true, differentialPrivacy: true, noiseEpsilon: 0.5 });
