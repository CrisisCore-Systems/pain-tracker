...
const svc = new PrivacyPreservingAnalyticsService({ dataRetentionDays: 30, differentialPrivacyEnabled: false });
...
describe('PrivacyAnalyticsService.calculateWeeklyAverage', () => {
  ...
  events.push({ eventType: 'pain_logged', timestamp: new Date(now), metadata: { painLevel: 8.5 }, sessionHash: 's1', isAnonymized: true });
  const oneDayAgo = new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000);
  events.push({ eventType: 'pain_logged', timestamp: oneDayAgo, metadata: { painLevel: 9 }, sessionHash: 's2', isAnonymized: true });
  const tenDaysAgo = new Date(now.getTime() - 10 * 24 * 60 * 60 * 1000);
  events.push({ eventType: 'pain_logged', timestamp: tenDaysAgo, metadata: { painLevel: 0 }, sessionHash: 's3', isAnonymized: true });
