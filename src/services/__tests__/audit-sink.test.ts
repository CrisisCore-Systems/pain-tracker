...
describe('InMemoryAuditSink', () => {
  it('appends events and signs them with the audit key', async () => {
    ...
    const evt = { eventId: '1', timestamp: new Date().toISOString(), eventType: 'TEST_EVENT', userIdHmac: 'hmac', details: { foo: 'bar' } };
    const signed = await sink.append(evt);
    expect(signed.signature).toBeDefined();
    // verify signature matches
    const serialized = JSON.stringify({ eventId: signed.eventId, timestamp: signed.timestamp, eventType: signed.eventType, userIdHmac: signed.userIdHmac, details: signed.details });
