...
function buildMoodEntry(overrides: Partial<MoodEntry> & { notes: string; context: string }): MoodEntry {
...
describe('EmpathyIntelligenceEngine (spike)', () => {
  ...
  it('elevates motivation and boundaries when supportive actions are recorded', async () => {
    const entries: MoodEntry[] = [
      buildMoodEntry({
        ...
        notes: 'I helped my neighbor recover after surgery, checked in later, and felt their pain while staying grounded.',
        energy: 8,
        hopefulness: 8,
      ...
      buildMoodEntry({
        ...
        notes: 'Volunteered and organized meals, encouraged the family, then recharged and protected my space.',
        energy: 7,
        stress: 3,
      ...
      buildMoodEntry({
        context: 'family call',
        notes: 'Listened deeply, mirrored feelings, and said no to an extra shift to protect my boundaries.',
        energy: 6,
        stress: 4,
  ...
  const metrics = await empathyIntelligenceEngine.calculateAdvancedEmpathyMetrics('supportive-user', [], entries);
    ...
    expect(metrics.emotionalIntelligence.neuralEmpathyPatterns.mirrorNeuronActivity).toBeGreaterThan(60);
    expect(metrics.emotionalIntelligence.neuralEmpathyPatterns.emotionalContagionResistance).toBeGreaterThan(60);
  });
  ...
  it('reduces boundary and motivation scores when overwhelmed without recovery', async () => {
    const entries: MoodEntry[] = [
      ...
      buildMoodEntry({
        ...
        notes: 'People pleasing took on too much, emotionally flooded, could not help myself afterwards.',
        anxiety: 7,
        stress: 7,
        energy: 4
      }),
  ...
  const metrics = await empathyIntelligenceEngine.calculateAdvancedEmpathyMetrics('overwhelmed-user', [], entries);
    ...
    expect(metrics.emotionalIntelligence.neuralEmpathyPatterns.emotionalContagionResistance).toBeLessThan(55);
  });
});
