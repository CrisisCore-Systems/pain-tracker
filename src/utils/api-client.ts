...
class CircuitBreaker {
  private f = 0; private t = 0; private s: 'CLOSED'|'OPEN'|'HALF' = 'CLOSED';
  constructor(private th=3, private win=30000, private half=1) {}
  get isClosed(){ this.chk(); return this.s==='CLOSED'; }
  success(){ this.f=0; this.s='CLOSED'; }
  failure(){ this.f++; this.t=Date.now(); if(this.s==='HALF'||this.f>=this.th) this.s='OPEN'; }
...
export const wcbApiBreaker = new CircuitBreaker();
  ...
  try {
    ...
    if (!res.ok) { wcbApiBreaker.failure(); throw new Error(`API ${res.status}`); }
    wcbApiBreaker.success();
    return await res.json() as T;
  } catch (e) { wcbApiBreaker.failure(); throw e; }
}
