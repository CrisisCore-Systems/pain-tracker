...
export const downloadData = (data: string, filename: string, mimeType: string = 'text/plain;charset=utf-8'): void => {
...
export const exportToPDF = (entries: PainEntry[]): string => {
  ...
  // Date range
  doc.setFontSize(12);
  const startDate = entries.length > 0 ? new Date(entries[0].timestamp).toLocaleDateString() : 'N/A';
  const endDate = entries.length > 0 ? new Date(entries[entries.length - 1].timestamp).toLocaleDateString() : 'N/A';
  ...
  doc.setFontSize(10);
  entries.slice(0, 20).forEach((entry, index) => { // Limit to first 20 entries for PDF
