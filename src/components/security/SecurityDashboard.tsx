...
export const SecurityDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'events' | 'metrics' | 'config'>(
    'overview'
  );
    ...
    try {
      ...
      const newMetrics: SecurityMetric[] = [
        ...
        },
        {
          ...
          status:
            status.recentEvents < 10 ? 'good' : status.recentEvents < 50 ? 'warning' : 'error',
        ...
        },
        {
          ...
          status:
            status.recentErrors === 0 ? 'good' : status.recentErrors < 5 ? 'warning' : 'error',
        ...
        },
        {
          ...
          status:
            configValidation.issues.length === 0
              ? 'good'
              : configValidation.issues.length < 3
                ? 'warning'
                : 'error',
        ...
        },
        {
          ...
          status:
            status.lastAudit && Date.now() - status.lastAudit.getTime() < 7 * 24 * 60 * 60 * 1000
              ? 'good'
              : 'warning',
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Real-time security monitoring and status
            </p>
          </div>
          <div className="text-right">
            <div
              className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(securityStatus.overall)}`}
            >
                ...
                {metrics.map((metric, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div
                        className={`text-2xl ${getStatusColor(metric.status)} w-12 h-12 rounded-full flex items-center justify-center`}
                      >
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p
                        className={`font-medium ${auditResult.passed ? 'text-green-600' : 'text-red-600'}`}
                      >
                    ...
                    <div className="mt-4">
                      ...
                      <div className="space-y-2">
                        {auditResult.issues.slice(0, 5).map((issue, index) => (
                          <div
                            key={index}
                            className="text-sm border-l-4 border-red-400 pl-4 py-2 bg-red-50"
                          >
              ...
              {recentEvents.length === 0 ? (
                ...
                <div className="space-y-2">
                  {recentEvents.map((event, index) => (
                    <div
                      key={index}
                      className={`border rounded-lg p-3 ${getStatusColor(event.level).replace('text-', 'border-').replace('bg-', 'bg-opacity-20 bg-')}`}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <span
                            className={`text-sm font-medium px-2 py-1 rounded ${getStatusColor(event.level)}`}
                          >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {metrics.map((metric, index) => (
                  ...
                  >
                    ...
                    <div
                      className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium mt-2 ${getStatusColor(metric.status)}`}
                    >
              ...
              <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                ...
                {(() => {
                  ...
                  return (
                    <>
                      <div
                        className={`inline-flex items-center px-2 py-1 rounded text-sm font-medium mb-4 ${getStatusColor(validation.isValid ? 'good' : 'error')}`}
                      >
