...
export function TreatmentComparison({ entries, className }: TreatmentComparisonProps) {
  ...
  useEffect(() => {
    if (!selectedTreatment || entries.length < 2) { setComparisonResult(null); return; }
    setIsComparing(true);
    (async () => {
      try {
      const baselineStart = new Date(Date.now() - (baselineDays + treatmentDays) * 24 * 60 * 60 * 1000);
      ...
      const treatmentEntries = entries.filter(e => {
        const d = new Date(e.timestamp);
        return d >= treatmentStart && d <= treatmentEnd && (e.medications?.current || []).some(m => m.name === selectedTreatment);
      });
      ...
      const cfg: TreatmentCfg = {
        ...
        beforeTreatment: { start: baselineStart, end: baselineEnd, entries: baselineEntries as unknown as PainEntry[] },
        afterTreatment: { start: treatmentStart, end: treatmentEnd, entries: treatmentEntries as unknown as PainEntry[] },
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <select className="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={selectedTreatment} onChange={(e) => setSelectedTreatment(e.target.value)}>
            ...
            <div className="space-y-2">
              ...
              <select className="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={baselineDays.toString()} onChange={(e) => setBaselineDays(parseInt(e.target.value))}>
            ...
            <div className="space-y-2">
              ...
              <select className="mt-1 block w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={treatmentDays.toString()} onChange={(e) => setTreatmentDays(parseInt(e.target.value))}>
                <option value="7">7 days</option>
                <option value="14">14 days</option>
                <option value="30">30 days</option>
                <option value="60">60 days</option>
          ...
          {availableTreatments.length === 0 && (
            <div className="flex items-center space-x-2 p-4 bg-muted rounded-lg">
              ...
              <p className="text-sm text-muted-foreground">
                No treatments found. Start adding treatments to your pain entries to enable comparisons.
              </p>
            </div>
          )}
        </CardContent>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{formatNumber(comparisonResult.baselineStats.averagePain, 1)}</p>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{formatNumber(comparisonResult.treatmentStats.averagePain, 1)}</p>
                    <p className="text-xs text-muted-foreground">
      ...
      {!selectedTreatment && availableTreatments.length > 0 && (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              Choose a treatment or medication from the dropdown above to see how it affects your pain levels.
            </p>
          </CardContent>
        </Card>
      )}
