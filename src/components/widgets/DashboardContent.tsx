...
export function DashboardContent({ entries, allEntries }: DashboardContentProps) {
  ...
  const metrics = useMemo(() => {
    ...
    const rolling = buildRolling7DayChartData(entries.map(e => ({ created_at: e.timestamp, pain_level: e.baselineData.pain }) as RawEntry), {} as any);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2"><BarChart3 className="h-5 w-5" /><span>Pain Distribution</span></CardTitle>
          </CardHeader>
          <CardContent>
            <Chart data={{ labels: ['Mild','Moderate','Severe','Extreme'], datasets: [{ label: 'Entries', data: metrics.painRanges }] }} type="bar" height={200} />
          </CardContent>
        </Card>

        <Card>
          <CardHeader>
            <CardTitle className="flex items-center space-x-2"><TrendingUp className="h-5 w-5" /><span>Weekly Trend</span></CardTitle>
          </CardHeader>
          <CardContent>
            <Chart data={{ labels: metrics.rolling.labels || [], datasets: metrics.rolling.datasets || [] }} type="line" height={200} />
          </CardContent>
        </Card>
      </div>
    </div>
