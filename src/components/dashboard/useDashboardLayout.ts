...
export function useDashboardLayout() {
  ...
  const normalizeColumns = useCallback(
    (value: DashboardLayout['columns'] | number | undefined): DashboardLayout['columns'] => {
      return value === 1 ? 1 : 2;
    },
    []
  );
  ...
  const normalizeLayout = useCallback(
    (rawLayout: DashboardLayout): DashboardLayout => ({
      ...rawLayout,
  ...
  const updateWidgetSize = useCallback(
    (widgetId: string, size: 'small' | 'medium' | 'large' | 'full') => {
      const newLayout = {
        ...layout,
        widgets: layout.widgets.map(widget =>
  ...
  const updateLayoutSettings = useCallback(
    (settings: Partial<DashboardLayout>) => {
      const newLayout = {
        ...layout,
        ...settings,
      };
