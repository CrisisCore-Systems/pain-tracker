...
export function PainEntryForm({ onSubmit }: PainEntryFormProps) {
  ...
  const [formData, setFormData] = useState<Omit<PainEntry, 'id' | 'timestamp'>>({
    ...
    // Optional numeric mood rating (0-10) for analytics; does not replace qualityOfLife.moodImpact
    mood: 5,
    workImpact: {
      missedWork: 0,
      modifiedDuties: [],
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm text-muted-foreground">
              {formData.mood ?? 0}/10 -
              {(formData.mood ?? 0) <= 3 ? ' Low' : (formData.mood ?? 0) <= 7 ? ' Neutral' : ' Good'}
            </span>
          </div>
          <div className="space-y-3">
            <input
              ...
              onChange={e => setFormData(prev => ({ ...prev, mood: parseInt(e.target.value, 10) }))}
              className="w-full h-2 bg-muted rounded-lg appearance-none cursor-pointer slider"
              aria-label={`Mood: ${formData.mood ?? 5} out of 10`}
            />
  ...
  const isSectionValid = (sectionIndex: number) => {
    if (sectionIndex === 0) {
      ...
      const hasLocation = (formData.baselineData.locations?.length ?? 0) > 0;
      return formData.baselineData.pain >= 0 && formData.baselineData.pain <= 10 && hasLocation;
    }
