...
module.exports = {
  ...
  theme: {
    extend: {
      colors: {
        ...
        biophilia: {
          bg1: 'hsl(var(--biophilia-bg-1))',
          bg2: 'hsl(var(--biophilia-bg-2))',
          accent: 'hsl(var(--biophilia-accent))',
          muted: 'hsl(var(--biophilia-muted))',
        },
      },
  ...
  plugins: [
    ...
    /* Accessibility helper: inject light-mode base rules for pale badge backgrounds
       so text on those backgrounds uses the foreground color and a stronger weight.
       This centralizes the fixes in Tailwind's theme system rather than ad-hoc
    ...
    require('tailwindcss/plugin')(function ({ addBase, addUtilities, theme }) {
      ...
      addUtilities(utilRules, { variants: [] });
    }),
  ],
};
